<template>
  ...
  <ion-list class="ion-padding">
    <post-filter-button
      v-if="$route.fullPath.includes('feed')"
      class="ion-margin-bottom ion-text-end"
        ...
        <ion-grid>
          <ion-row>
            <ion-col size="10">
              <ion-list
                v-if="
                  (post.tags && post.tags.length > 0) || (post.triggers && post.triggers.length > 0)
                "
                class="ion-padding-bottom"
              >
                <ion-chip
                  v-for="element in [...post.tags, ...post.triggers]"
                  :key="element.id"
              ...
              <ion-item lines="none" class="ion-no-shadow ion-align-items-start">
                ...
                <div class="ion-padding-top">
                  ...
                  <p>
                    <ion-text class="ion-margin-top post-content" :auto-grow="true">
                  ...
                  <ion-text
                    ...
                    @click.stop="toggleOriginal(post)"
                  >
                    Voir l'original
                  </ion-text>
                </div>
          ...
          <ion-row>
            <ion-col size="auto">
              <ion-icon
                class="custom-icon"
                @click.stop="openCommentModal(post.id)"
      ...
      <ion-item lines="none" class="no-posts">
        <ion-grid>
          <ion-row class="ion-justify-content-center">
            <ion-col size="12">
              <ion-label>
                <h2 class="font-bold">Aucun post ne correspond aux filtres sélectionnés</h2>
                <p>
                  Essayez de modifier vos critères de recherche ou supprimez les filtres pour voir
                  plus de contenu.
                </p>
              </ion-label>
            </ion-col>
          ...
          <ion-row class="ion-justify-content-center">
            <ion-col size="12">
              <ion-label>
                ...
                <p>
                  Exprimez ce que vous ressentez, anonymement et en toute liberté. Votre voix compte
                  ici.
                </p>
              </ion-label>
            </ion-col>
    ...
    filteredPosts() {
      return this.posts.filter((post) => {
        const hasSelectedTags =
          ...
          post.tags.some((tag) => this.selectedTags.includes(tag.id))
        const hasSelectedTriggers =
          this.selectedTriggers.length === 0 ||
          post.triggers.some((trigger) => this.selectedTriggers.includes(trigger.id))
        return hasSelectedTags && hasSelectedTriggers
      })
    },
    paginatedPosts() {
      return this.filteredPosts.slice(0, this.currentPage * this.postsPerPage)
    }
  },
  methods: {
    timeSince,
