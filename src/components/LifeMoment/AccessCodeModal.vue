  ...
  <ion-modal :is-open="isOpen" :backdrop-dismiss="true">
    <ion-card class="ion-padding ion-no-shadow">
      <ion-label position="stacked">{{
        hasAccessCode ? "Entrer le code d'accès" : "Définir un code d'accès"
      }}</ion-label>
      <ion-input minlength="4" v-model="accessCode" type="password" inputmode="numeric"></ion-input>
      <div class="ion-text-end">
        <custom-button
          :disabled="accessCode.length < 5"
          :text="hasAccessCode ? 'Entrer' : 'Définir'"
          @click="handleAccessCode"
        ></custom-button>
      </div>
    </ion-card>
  </ion-modal>
...
import { IonModal, IonLabel, IonInput, IonCard } from '@ionic/vue'
